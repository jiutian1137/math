...
/// 
///@license Please identify author, 2020-2022
///@author LongJiangnan, Jiang1998Nan@outlook.com
///@readme
/// floating<..>::frexp and ldexp are not perfect, because we don't have memory-operation functions.
/// In next optimization, we will add dynamic something and will have memory-operation functions.
///
/// 'floating<..>::cvtui()' and 'operator Integer()' are not perfect.
#pragma once
#ifndef _MATH_BEGIN
 ...
 *
 *     00011000 A2 ^ B2      = A3
 *     00000000 A3 & B2, so 'A3' is Result .
 * 
 * @overflow
 *     00000000 A
 *   - 01001000 B
 ...
 *     11011000 A1 ^ B1      = A2
 *   - 00100000 (A2 & B1)<<1 = B2
 *   = 216-32 = 184(10digit), overflow
 * 
 *   AN must less A if A >= B.
 *   AN^BN must eliminate final_bit if A[final] == B[final] == 1, example 1...^1... = 0...., so it is not overflow.
 *   so, if we will shift B and B[final]==1 (now we not eliminate final_bit), will overflow. 
*/
	for ( ; ; ) {
		_Left  = _Left ^ _Right;
...
public:
	...
	static void do_cvtf(const InBitset& source, Bitset& destination) {
		typedef floating<s2, e2, InBitset> InFloat;
		typedef floating<s, e, Bitset> Float;
...
public:
	...
	explicit floating(const Bitset& _bitset) : _Mybitset(_bitset) {}

public:
	floating() : _Mybitset() {}

	template<size_t s2, size_t e2, typename InBitset> 
	explicit floating(const floating<s2,e2,InBitset>& other) {
		do_cvtf<s2, e2>(other.bitset(), this->bitset());
	}
		
	floating(float other) {
		do_cvtf<23, 8>(reinterpret_cast<const std::bitset<32>&>(other), this->bitset());
	}
		
	floating(double other) {
		do_cvtf<52, 11>(reinterpret_cast<const std::bitset<64>&>(other), this->bitset());
	}

	floating(long double other) : floating(static_cast<double>(other)) {}

	floating(unsigned int other) {
		do_cvtui(std::bitset<sizeof(other)*8>(other), this->bitset());
	}

	floating(unsigned long long other) {
		do_cvtui(std::bitset<sizeof(other)*8>(other), this->bitset());
	}
	...
	explicit operator float() const {
		auto the_float = floating<23,8>(*this);
		return reinterpret_cast<const float&>(the_float);
	}

	explicit operator double() const {
		auto the_double = floating<52,11>(*this);
	...
	explicit operator unsigned long long() const {
		...
		if (saved_bits > sizeof(unsigned long long) * 8) {
			throw std::overflow_error("floating<...>::operator unsigned int() const");
		}
	...
	bool operator==(const floating& right) const {
	...
	floating operator-() const {
		return floating{ ((_Mybitset & sign_mask()) ^ sign_mask()) | (_Mybitset & (~sign_mask())) };
	}

	floating& operator+=(const floating& right) {
	...
	floating operator+(const floating& right) const {
		return floating(*this) += right;
	}
...
template<typename Bitset>
class floating<23,8,Bitset,true> {
public:
...
public:
	...
	explicit floating(const floating<s2,e2,InBitset,IsOpt>& other) : _Myfp(floating<23,8,Bitset,false>(other).as<float>()) {}
	floating& operator=(floating&&) = default;
	floating& operator=(const floating&) = default;
	template<size_t s2, size_t e2, typename InBitset, bool IsOpt>
	floating& operator=(const floating<s2,e2,InBitset,IsOpt>& other) { 
		_Myfp = floating<23,8,Bitset,false>(other).as<float>(); 
		return *this; 
	}

	constexpr floating(float other) : _Myfp(other) {}
	constexpr floating(double other) : floating(static_cast<float>(other)) {}
	constexpr floating(long double other) : floating(static_cast<float>(other)) {}
	constexpr floating(bool other) : floating(static_cast<float>(other)) {}
	constexpr floating(int other) : floating(static_cast<float>(other)) {}
	constexpr floating(long long other) : floating(static_cast<float>(other)) {}
	constexpr floating(unsigned int other) : floating(static_cast<float>(other)) {}
	constexpr floating(unsigned long long other) : floating(static_cast<float>(other)) {}
	...
	inline constexpr bool operator==(floating right) const {
	...
	inline constexpr floating operator+() const {
		return floating{ _Myfp };
	}
	inline constexpr floating operator-() const {
		return floating{ -_Myfp };
	}
	inline constexpr floating operator+(floating right) const {
		return floating{ _Myfp + right._Myfp };
	}
	inline constexpr floating operator-(floating right) const {
		return floating{ _Myfp - right._Myfp };
	}
	...
	inline floating operator%(floating right) const {
		return floating{ _Myfp - _CSTD floor(_Myfp / right._Myfp) * right._Myfp };
		//return _CSTD fmodf(_Myfp, right._Myfp);
	}

	inline floating& operator+=(floating right) {
		_Myfp += right._Myfp;
		return *this;
	}
	inline floating& operator-=(floating right) {
...
template<typename Bitset>
class floating<52,11,Bitset,true> {
public:
...
public:
	...
	floating(const floating&) = default;
	template<size_t s2, size_t e2, typename InBitset, bool IsOpt>
	explicit floating(const floating<s2,e2,InBitset,IsOpt>& other) : _Myfp( floating<52,11,Bitset,false>(other).as<double>() ) {}
	floating& operator=(floating&&) = default;
	floating& operator=(const floating&) = default;
	template<size_t s2, size_t e2, typename InBitset, bool IsOpt>
	floating& operator=(const floating<s2,e2,InBitset,IsOpt>& other) {
		_Myfp = floating<52,11,Bitset,false>(other).as<double>();
		return *this;
	}

	constexpr floating(double other) : _Myfp(other) {}
	constexpr floating(float other) : floating(static_cast<double>(other)) {}
	constexpr floating(long double other) : floating(static_cast<double>(other)) {}
	constexpr floating(bool other) : floating(static_cast<double>(other)) {}
	constexpr floating(int other) : floating(static_cast<double>(other)) {}
	constexpr floating(long long other) : floating(static_cast<double>(other)) {}
	constexpr floating(unsigned int other) : floating(static_cast<double>(other)) {}
	constexpr floating(unsigned long long other) : floating(static_cast<double>(other)) {}
...
floating<s,e,b,opt> operator##_OP_##(const floating<s,e,b,opt>& left, _LITERAL_TYPE_ right) { \
	return left _OP_ static_cast< floating<s,e,b,opt> >(right); \
}

#define __calculation_float_operator_with_literal2(_OP_, _LITERAL_TYPE_) \
template<size_t s, size_t e, typename b, bool opt> inline \
floating<s,e,b,opt> operator##_OP_##(_LITERAL_TYPE_ left, const floating<s,e,b,opt>& right) { \
	return static_cast< floating<s,e,b,opt> >(left) _OP_ right; \
}
...
template<size_t s, size_t e, typename b, bool opt> inline \
floating<s,e,b,opt>& operator##_OP_##(floating<s,e,b,opt>& left, _LITERAL_TYPE_ right) { \
	return left _OP_ static_cast< floating<s,e,b,opt> >(right); \
}

#define __calculation_float_comparison_with_literal_commutatibity(_OP_, _LITERAL_TYPE_) \
template<size_t s, size_t e, typename b, bool opt> inline \
bool operator##_OP_##(const floating<s,e,b,opt>& left, _LITERAL_TYPE_ right) { \
	return left _OP_ static_cast< floating<s,e,b,opt> >(right); \
} \
template<size_t s, size_t e, typename b, bool opt> inline \
bool operator##_OP_##(_LITERAL_TYPE_ left, const floating<s,e,b,opt>& right) { \
	return static_cast< floating<s,e,b,opt> >(left) _OP_ right; \
} 
...
bool isinf(const floating<m,e,b>& x) {
	return floating<m,e,b>::do_isinf(x.bitset());
}  

template<size_t m, size_t e, typename b> inline
bool isnan(const floating<m,e,b>& x) {
	return floating<m,e,b>::do_isnan(x.bitset());
}

template<size_t m, size_t e, typename b> inline
floating<m,e,b> abs(const floating<m,e,b>& x) {
	auto abs_mask = ~(floating<m,e,b>::sign_mask());
	return floating<m,e,b>{ x.bitset() & abs_mask };
}

template<size_t s, size_t e, typename b> inline
floating<s,e,b> trunc(const floating<s,e,b>& x) {
	floating<s,e,b> ipart;
	floating<s,e,b> fpart;
	floating<s,e,b>::do_modf(x.bitset(), fpart.bitset(), ipart.bitset());
	return ipart;
}

template<size_t s, size_t e, typename b> inline
floating<s,e,b> modf(const floating<s,e,b>& x, floating<s,e,b>* ipart) {
	floating<s,e,b> fpart;
	floating<s,e,b>::do_modf(x.bitset(), fpart.bitset(), ipart->bitset());
	return fpart;
}
...
floating<s,e,b> frexp(const floating<s,e,b>& x, Integer* exp2) {
	floating<s,e,b> base;
	floating<s,e,b>::do_frexp(x.bitset(), base.bitset(), *exp2);
	return base;
}

template<size_t s, size_t e, typename b, typename Integer> inline
floating<s,e,b> ldexp(const floating<s,e,b>& base, const Integer& exp2) {
	floating<s,e,b> y;
	floating<s,e,b>::do_ldexp(base.bitset(), exp2, y.bitset());
	return y;
}

template<size_t m, size_t e, typename b> inline
floating<m,e,b> floor(const floating<m,e,b>& x) {
	if ( x >= 0 ) {
		return trunc(x);
	} else {
		return trunc(x) - 1;
...
template<size_t m, size_t e, typename b> inline
floating<m,e,b> ceil(const floating<m,e,b>& x) {
	return floor(x) + 1;
}

template<size_t m, size_t e, typename b> inline
floating<m,e,b> round(const floating<m,e,b>& x) {
	return floor(x + 0.5);
}

template<size_t s, size_t e, typename b> inline
floating<s,e,b> fmod(const floating<s,e,b>& x, const floating<s,e,b>& y) {
...
using float32_t = floating<23, 8, std::bitset<32>, true>;
/** IEEE754 double-precision */
using float64_t = floating<52, 11, std::bitset<64>, true>;
/** @see GCC/quadmath/ALL */
using float128_t = floating<112, 15>;
using float256_t = floating<235, 20>;
using float512_t = floating<485, 26>;
using float1024_t = floating<992, 31>;

inline bool isinf(float32_t x) { return std::isinf(x); }
inline bool isinf(float64_t x) { return std::isinf(x); }
inline bool isnan(float32_t x) { return std::isnan(x); }
inline bool isnan(float64_t x) { return std::isnan(x); }
inline float32_t abs(float32_t x) { return std::fabsf(x); }
inline float64_t abs(float64_t x) { return std::fabs(x); }
inline float32_t trunc(float32_t x) { return std::truncf(x); }
inline float64_t trunc(float64_t x) { return std::trunc(x); }
inline float32_t modf(float32_t x, float32_t* i) { return std::modf(x,(float*)i); }
inline float64_t modf(float64_t x, float64_t* i) { return std::modf(x,(double*)i); }
inline float32_t frexp(float32_t x, int* ep) { return std::frexp(x, ep); }
inline float64_t frexp(float64_t x, int* ep) { return std::frexp(x, ep); }
inline float32_t ldexp(float32_t x, int ep) { return std::ldexp(x, ep); }
inline float64_t ldexp(float64_t x, int ep) { return std::ldexp(x, ep); }
inline float32_t floor(float32_t x) { return std::floorf(x); }
inline float64_t floor(float64_t x) { return std::floor(x); }
inline float32_t ceil(float32_t x) { return std::ceilf(x); }
inline float64_t ceil(float64_t x) { return std::ceil(x); }
inline float32_t round(float32_t x) { return std::roundf(x); }
inline float64_t round(float64_t x) { return std::round(x); }
inline float32_t fmod(float32_t x, float32_t y) { return std::fmodf(x, y); }
inline float64_t fmod(float64_t x, float64_t y) { return std::fmod(x, y); }

inline float32_t sqrt(float32_t x) { return std::sqrtf(x); }
inline float64_t sqrt(float64_t x) { return std::sqrt(x); }
inline float32_t cbrt(float32_t x) { return std::cbrtf(x); }
inline float64_t cbrt(float64_t x) { return std::cbrt(x); }
inline float32_t exp(float32_t x) { return std::expf(x); }
inline float64_t exp(float64_t x) { return std::exp(x); }
inline float32_t log(float32_t x) { return std::logf(x); }
inline float64_t log(float64_t x) { return std::log(x); }
inline float32_t pow(float32_t x, float32_t power) { return std::powf(x, power); }
inline float64_t pow(float64_t x, float64_t power) { return std::pow(x, power); }
inline float32_t sin(float32_t x) { return std::sinf(x); }
inline float64_t sin(float64_t x) { return std::sin(x); }
inline float32_t cos(float32_t x) { return std::cosf(x); }
inline float64_t cos(float64_t x) { return std::cos(x); }
inline float32_t tan(float32_t x) { return std::tanf(x); }
inline float64_t tan(float64_t x) { return std::tan(x); }
inline float32_t asin(float32_t x) { return std::asinf(x); }
inline float64_t asin(float64_t x) { return std::asin(x); }
inline float32_t acos(float32_t x) { return std::acosf(x); }
inline float64_t acos(float64_t x) { return std::acos(x); }
inline float32_t atan(float32_t x) { return std::atanf(x); }
inline float64_t atan(float64_t x) { return std::atan(x); }
inline float32_t atan2(float32_t y, float32_t x) { return std::atan2f(y,x); }
inline float64_t atan2(float64_t y, float64_t x) { return std::atan2(y,x); }
_MATH_END
...
#include <xstring>
_MATH_BEGIN
// { used for floating<X,X> to decimal to decimal_string }
struct decimal {
...
std::string to_string(floating<m,e,b,isopt> _Source) {
	using floatX_t = floating<m, e, b, isopt>;
...
std::ostream& operator<<(std::ostream& _Ostr, floating<m,e,Bitset,IsOpt> _Fp) {
...
class numeric_limits< math::floating<m,e,b,opt> > : public _Num_float_base{
public:
	_NODISCARD static constexpr math::floating<m,e,b,opt>(min)() noexcept {
		abort();
	}

	_NODISCARD static constexpr math::floating<m,e,b,opt>(max)() noexcept {
		abort();
	}

	_NODISCARD static constexpr math::floating<m,e,b,opt> lowest() noexcept {
		abort();
	}

	_NODISCARD static math::floating<m,e,b,opt> epsilon() noexcept {
		return math::floating<m,e,b,opt>( math::floating<m,e,b,opt>::epsilon_bitset() );
	}

	_NODISCARD static math::floating<m,e,b,opt> round_error() noexcept {
		return epsilon() / 2;
	}

	_NODISCARD static constexpr math::floating<m,e,b,opt> denorm_min() noexcept {
		abort();
	}

	_NODISCARD static math::floating<m,e,b,opt> infinity() noexcept {
		return math::floating<m,e,b,opt>( math::floating<m,e,b,opt>::inf_bitset() );
	}

	_NODISCARD static math::floating<m,e,b,opt> quiet_NaN() noexcept {
		return math::floating<m,e,b,opt>( math::floating<m,e,b,opt>::quiet_NaN_bitset() );
	}

	_NODISCARD static math::floating<m,e,b,opt> signaling_NaN() noexcept {
		return math::floating<m,e,b,opt>( math::floating<m,e,b,opt>::signaling_NaN_bitset() );
	}

	static constexpr size_t digits = math::floating<m,e,b,opt>::significand_bits + 1;
};

/*template<size_t s, size_t e, typename b, bool opt>
_INLINE_VAR constexpr bool is_floating_point_v<math::floating<s,e,b,opt>> = true*/;
_STD_END
